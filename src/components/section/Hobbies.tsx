...
const Hobbies = ({
  ...
  return (
    <section id="hobby" className="bg-red-700 px-20 py-36">
      <div className="w-[500px]">
        <h2>
          <motion.span
            style={{ transform: translateValue, opacity }}
            className="inline-block transition-all duration-700 ease-in-out text-5xl font-black text-sky-50 mr-4"
          >
            HOBBY
          </motion.span>
          <motion.span
            style={{ transform: translateValue, opacity }}
            className="inline-block transition-all duration-700 ease-in-out text-5xl font-black"
          >
            PROJECTS
          </motion.span>
        </h2>
        ...
        <motion.p
          style={{ opacity }}
          className="transition-all duration-500 ease-in-out text-sky-50/90 tracking-widest font-roboto mt-4 whitespace-break-spaces"
        >
      ...
      <div className="mx-auto my-10 flex flex-col items-center justify-center border-t-3 border-sky-50 border-t-solid">
        {music.map((music, index) => (
          <a
            ...
            href={music.src}
            style={{}}
            className="group w-full py-8 px-5 border-b-3 border-sky-50 border-b-solid"
          >
            <div className="relative z-0 flex justify-between items-center text-sky-50">
              <h3 className="w-1/2 text-4xl py-2 font-bold">{music.name}</h3>
              <span className="w-1/4 text-right font-light text-xl text-sky-50/80">
                {music.type}
              </span>
              <span className="w-1/4 text-right text-6xl">{"⇗"}</span>
              <div
